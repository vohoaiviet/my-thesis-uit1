...
\item \textbf{Lớp Decoder và Encoder:} giải mã và mã hóa hình ảnh.
...
\item \textbf{Lớp MenuView:} cung cấp các phím chức năng cho phép người dùng thao tác với ứng dụng, gọi bộ lắng nghe sự kiện ActionListener khi người dùng thực hiện các chức năng để tương tác với các thành phần khác.
...
\begin{figure}[!htbp]
  \begin{center}
    \leavevmode
    \ifpdf
      \includegraphics[scale=0.4]{interface_1}
    \else
      \includegraphics[scale=0.4]{interface_1}
    \fi
